...
def main() -> None:
    ...
    if gender_filter not in {"both", "m", "w"}:
        raise ValueError(
            f"Invalid NCAA_GENDER_FILTER={gender_filter!r}. Expected one of: both, m, w"
        )
    run_m = gender_filter in {"both", "m"}
    run_w = gender_filter in {"both", "w"}
    ...
    if phase6_enabled:
        ...
        try:
            ...
            if run_m and not m_graph_features.empty:
                m_graph_features.to_csv(FEATURES_DIR / "graph_features_m.csv", index=False)
            if run_w and not w_graph_features.empty:
    ...
    m_oof = pd.DataFrame()
    w_oof = pd.DataFrame()
    m_fold_metrics = pd.DataFrame()
    w_fold_metrics = pd.DataFrame()
    m_bins = pd.DataFrame()
    w_bins = pd.DataFrame()
    ...
    # Console summary
    print("\n=== Pipeline complete ===")
    print(f"Run mode: NCAA_GENDER_FILTER={gender_filter}")
    if run_m:
